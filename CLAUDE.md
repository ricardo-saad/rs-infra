...
- `.github/workflows/static-validation.yml` — the only CI workflow that exists; static/fork-safe checks only.
...
- Build tool: Packer (`packer { required_version >= 1.10.0 }`), single source `amazon-ebs.gateway`, ARM64 (`t4g.micro` default), defined in `images/gateway/gateway.pkr.hcl`.
...
- `scripts/build.sh` also verifies the source AMI's owner/architecture/ID before invoking Packer, and pins the installed Packer binary version against `RS_GATEWAY_PACKER_VERSION`.
- `scripts/install.sh` (runs as root inside the Packer builder via `provisioner "shell"`) installs pinned apt packages from a single pinned repository line, downloads+SHA256-verifies the AdGuard Home archive, copies `rootfs/` onto `/` with `cp -a`, sets exact permissions (`0755` on `*.py`/`*.sh`, `0644` on schemas, `0600` on `runtime.env`), and enables `rs-gateway.target` while disabling `rs-gateway-bootstrap.service` and the stock `wg-quick@*` units.
...
`.github/workflows/static-validation.yml` runs on every pull request (unfiltered — any target branch, so stacked branches are covered) and on every push to `main` (post-merge validation of the integrated result). It holds `permissions: contents: read` only — no OIDC, no provider credentials, no Terraform state, no Talos/Kubernetes access — so it is safe on forked PRs. In-progress runs are superseded only for pull requests; `main` runs always complete, because their result is the record that the merged state passed. Jobs:
