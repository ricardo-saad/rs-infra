...
Independent Terraform stack reserved for the versioned S3 state foundation,
GitHub OIDC providers, and the plan/apply CI roles.

The backend block is deliberately partial. Supply its configuration during
`terraform init` only after the state naming and bootstrap sequence are agreed.
This scaffold creates no identity or state resources yet.
...
| <a name="input_aws_region"></a> [aws\_region](#input\_aws\_region) | AWS region for regional bootstrap resources. | `string` | `"eu-west-2"` | no |
